...
$$ \text{GV}_\alpha(p, \hat{p}) = \frac {1} {\sqrt[\alpha]{2}}  {\| p - \hat{p} \|}_\alpha =  \sqrt[\alpha]{{ \sum_l{ \frac {1}{2} ( p_{l} - \hat{p}_{l})^\alpha } }} ,\quad  \alpha \in [1,\infty]$$
...
Finally, the case for $\alpha = \infty$, $\lim_{\alpha\to\infty} \text{GV}_\alpha(p, \hat{p}) = \| p - \hat{p} \|_\infty $ between $p$ and $\hat{p}$. The reason is that for $\alpha \gg 1$ the greatest difference dominates in the sum $\sum_l{ {| p_l - \hat{p}_l |}^\alpha }$. Therefore $\lim_{\alpha\to\infty} \| p - \hat{p} \|_\alpha \approx \max_l { | p_l - \hat{p}_l | } $.
